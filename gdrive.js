...
function setSyncingIndicator(isSyncing) {
  const syncNowBtn = document.getElementById('gdriveSyncNowBtn');
  if (syncNowBtn) {
    if (isSyncing) {
      syncNowBtn.innerText = "🔄 Syncing...";
      syncNowBtn.style.pointerEvents = "none";
      syncNowBtn.style.opacity = "0.7";
    } else {
      syncNowBtn.innerText = "🔄 Sync Now";
      syncNowBtn.style.pointerEvents = "auto";
...
function updateSyncUI() {
  ...
  const widget = document.getElementById('gdriveSyncWidget');
    ...
    sidebarContainer.innerHTML = `
      <div style="display:flex; flex-direction:column; gap:0.5rem; width:100%;">
        <div style="display:flex; justify-content:space-between; align-items:center;">
          ...
          <button id="gdriveSyncNowBtn" style="background:transparent; border:none; color:#4285F4; font-size:0.75rem; font-weight:600; cursor:pointer;" title="Sync Now">🔄 Sync Now</button>
        </div>
        <button id="gdriveLogoutBtn" class="btn btn-secondary" style="width:100%; font-size:0.75rem; padding:0.4rem; border-color:var(--accent-expense); color:var(--accent-expense);">Disconnect</button>
      </div>
    `;

    document.getElementById('gdriveLogoutBtn').addEventListener('click', () => {
      if (confirm("Are you sure you want to disconnect Google Drive sync and wipe all local storage data from this device?")) {
        handleLogout();
      }
    });
    document.getElementById('gdriveSyncNowBtn').addEventListener('click', startSync);
  } else {
    // If client ID is present, we show the sign-in widget.
    // Otherwise, we show a button prompting to configure.
    if (clientId) {
      sidebarContainer.innerHTML = `
        <div style="display:flex; flex-direction:column; gap:0.5rem; width:100%;">
          <div class="gdrive-status-badge disconnected">● Disconnected</div>
          <button id="gdriveLoginBtn" class="btn btn-primary" style="width: 100%; font-size: 0.8rem; padding: 0.5rem; background: #4285F4; display: flex; align-items: center; justify-content: center; gap: 0.5rem; border: none; box-shadow: 0 4px 10px rgba(66, 133, 244, 0.2);">
      ...
      `;
      document.getElementById('gdriveLoginBtn').addEventListener('click', handleAuthClick);
    } else {
      sidebarContainer.innerHTML = `
        <div style="display:flex; flex-direction:column; gap:0.5rem; width:100%;">
          <div class="gdrive-status-badge disconnected" style="font-size: 0.7rem;">● Setup Required</div>
          <button id="gdriveSetupBtn" class="btn btn-secondary" style="width: 100%; font-size: 0.75rem; padding: 0.4rem;">Configure ID</button>
        </div>
      `;
      document.getElementById('gdriveSetupBtn').addEventListener('click', () => {
        switchTab('backup');
        const input = document.getElementById('gdriveClientId');
        if (input) input.focus();
      });
